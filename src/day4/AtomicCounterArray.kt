...
class AtomicCounterArray(size: Int) {
    ...
    fun inc2(index1: Int, index2: Int) {
        ...
        while (true) {
            val cur1 = get(index1)
            val cur2 = get(index2)
            val descriptor = IncrementDescriptor(index1, cur1, index2, cur2)
    ...
    private inner class IncrementDescriptor(
        ...
        fun applyOperation() {
            if (status.value == UNDECIDED) {
                ...
                val test1 = array[index1].compareAndSet(valueBeforeIncrement1, this)
                val val2 = array[index2].value
                val test2 = array[index2].compareAndSet(valueBeforeIncrement2, this)
                if (val1 != this && !test1 || val2 != this && !test2) {
